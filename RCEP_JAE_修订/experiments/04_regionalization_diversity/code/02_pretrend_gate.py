#!/usr/bin/env python3
"""Estimate the frozen high-dimensional-FE event-study pretrend gate."""
...
def main() -> None:
    panel = pd.read_parquet(DATA / "firm_country_year.parquet")
    if panel.duplicated(["firm_country_id", "year"]).any():
        raise ValueError("Firm-country-year panel has duplicate keys.")

    work = panel.copy()
    cohort_sizes = (
        work.loc[work["rcep"].eq(1), ["firm_country_id", "entry_policy_year"]]
        .drop_duplicates()
        .groupby("entry_policy_year")["firm_country_id"]
        .nunique()
        .to_dict()
    )
    terms: list[str] = []
    ...
    y = work["asinh_active_supplier_links"].to_numpy(dtype=float)
    x = work[terms].to_numpy(dtype=float)
    fixed_effects = work[["firm_country_id", "firm_year_id"]].to_numpy(dtype=np.int64)
    ...
    for event_time in sorted({item["event_time"] for item in meta}):
        ...
        raw_weights = np.array([cohort_sizes[item["group"]] for item in selected], dtype=float)
    ...
    export_table(event, "table2_event_study_gate", "Supplier-Link Reallocation Event Study")

    FIGURES.mkdir(parents=True, exist_ok=True)
    trends = (
        work.groupby(["year", "rcep"], as_index=False)["asinh_active_supplier_links"]
        .mean()
        .pivot(index="year", columns="rcep", values="asinh_active_supplier_links")
    )
    ...
    ax.set_xlabel("Year")
    ax.set_ylabel("Mean asinh active supplier links")
    ...
    ax.set_ylabel("Difference in asinh active supplier links")
    ax.grid(axis="y", color="#D9D9D9", linewidth=0.5)
    fig.tight_layout()
    ...
    diagnostics = {
        ...
        "firm_country_fixed_effects": int(work["firm_country_id"].nunique()),
        "firm_year_fixed_effects": int(work["firm_year_id"].nunique()),
        "partner_country_clusters": cluster_count,
        "estimator": "OLS after pyhdfe residualization on firm-country and firm-year fixed effects",
    }
    ...
    status["next_gate"] = "STATIC_EFFECT_AND_RANDOMIZATION" if gate_pass else "ARCHIVE_FAILURE"
